...
\path{fix/vue-cal-xss-safe-rendering} ist der sichere Renderer direkt
in den regulären Ansichten umgesetzt; eine Laufzeitumschaltung zurück
zur anfälligen Variante ist dort nicht vorhanden. Die Umsetzung ist in
Commit \texttt{08567da9} mit der Nachricht
\texttt{fix: render VueCal events safely} dokumentiert.

\subsection{Bewertung des Fix-Branches}

Nach der vollständigen Umsetzung und einer erneuten Prüfung aller
relevanten Kalenderansichten auf
\path{fix/vue-cal-xss-safe-rendering} besteht dort kein bekanntes
aktuelles XSS-Risiko über den \texttt{vue-cal}-Standardrenderer. Der
Branch ist bereit für den Merge in \texttt{capstone-main}. Bis zu
diesem Merge weisen \texttt{capstone-main} und sämtliche anderen zum
Berichtsstand geprüften Remote-Branches das ursprüngliche anfällige
